...
@Component({
  selector: 'app-rapportc',
  templateUrl: './rapportc.component.html',
  styleUrls: ['./rapportc.component.css'],
  providers: [{
    provide: STEPPER_GLOBAL_OPTIONS, useValue: { showError: true }
  }]
})
export class RapportcComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ConsommableArray.valueChanges.subscribe(val => {
      if (this.ConsommableArray.controls[this.index3].get('consommable').value)
       { for (let i of this.consommables) {
          if (i.nom == val[this.index].consommable)
          {
    ...
    this.registerForm1.get('equipement').valueChanges.subscribe(val => {
      this.registerForm1.get('prestation').setValue(val.prestation);
      this.p = val.prestation.nom;
  ...
  gotoList() {
    ...
    this.router.navigate(['/rapport_journalier']);
  }
  getChantiers() {
